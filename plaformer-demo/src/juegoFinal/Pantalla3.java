...
/**
 * La clase que crea y maneja la pantalla 3
 * @author Marvin Gonzalez, Kevin Gutierrez, Néstor Villalobos
 */
public class Pantalla3 extends funScreen {
    ...
    public void crear() {
        setPuntaje(0);
        setVidas(3);
        setEscenario(new Escenario("assets/mapa3.csv"));
        setJugador1(new Player1(16 * 2, 16 * 2, 100, 0, "assets/player1.png", 16, 16));
        setJugador2(new Player2(16 * 3, 18 * 3, 70, 0, "assets/player2.png", 16, 18));
        setFondo(new Fondo(800, 600, 0, 0, "assets/fondo1.png", 600, 400));
        setEstrellas(new Estrellas3());
        setTexto(new Texto());
        ...
        for (funTile tile : getEscenario().getTiles()) {
            if (tile.getId() == 23) setPuerta(tile);
        }
    }
    ...
    public void controlColisiones() {
        if (funLaws.getFunLaws().existeColisionEntre(getPuerta(), getJugador2())) {
            ((Pantalla4)getPadre().getPantallas().get(3)).setPuntaje(getPuntaje());
            ((Pantalla4)getPadre().getPantallas().get(3)).setVidas(getVidas());
            getPadre().ejecutarPantalla(getPadre().getPantallas().get(3));
        }
     ...
     */
    public Estrellas3 getEstrellas() {
        return estrellas;
    }
     ...
     */
    public void setEstrellas(Estrellas3 estrellas) {
